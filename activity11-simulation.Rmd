...
It would be nice to explore multiple distributions simultaneously.
...
generate_n_samples <- function(n) {
    norm_samp <- rnorm(n = n, mean = 45, sd = 5)

    exp_samp <- rexp(n = n, rate = .05)

    chi_samp <- rchisq(n = n, df = 3)

    uniform_samp <- runif(n = n, min = 0, max = 10)
...
```{r}
sample_mean(generate_n_samples(n=30))
```
